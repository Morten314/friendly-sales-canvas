...
def create_from_suggested_icp(driver, mongo, request: SuggestedICPToCustomerProfileRequest) -> dict:
    ...
    for item in suggested:
        if isinstance(item, dict) and str(item.get("id")) == str(request.icp_id):
            ...
            break
    if not target:
        raise SuggestedICPNotFoundError(f"Suggested ICP not found for icp_id: {request.icp_id}")
    ...
    for existing in existing_icps:
        if isinstance(existing, dict) and str(existing.get("source_suggested_icp_id")) == str(request.icp_id):
            raise ICPAlreadyExistsError("This suggested ICP is already saved in customer profile.")
...
def delete_icp_from_customer_profile(mongo, icp_id: str, org_id: str) -> dict:
    ...
    document = collection.find_one(filter_query)
    if not document:
        raise CustomerProfileNotFoundError(f"No customer profile document found for org_id: {org_id}")
    ...
    if not deleted_icp:
        raise CustomerProfileICPNotFoundError(f"Customer profile ICP not found for icp_id: {icp_id}")
