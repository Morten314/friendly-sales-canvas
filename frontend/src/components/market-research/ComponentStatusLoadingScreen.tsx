...
export const ComponentStatusLoadingScreen: React.FC<ComponentStatusLoadingScreenProps> = ({
  ...
  const components: ComponentStatus[] = [
    ...
    { name: "Market Entry", status: componentStatus["Market Entry"], icon: MapPin },
    {
      ...
      status: componentStatus["Competitor Landscape"],
      icon: Building2,
    },
    {
      name: "Regulatory Compliance",
      status: componentStatus["Regulatory Compliance"],
      icon: Shield,
    },
  ];
  ...
  const getOverallStatus = () => {
    if (loadingPhase === "rendering") {
      const renderedCount = Object.values(componentRenderingStatus).filter(
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-8 max-w-2xl mx-4 w-full max-h-[80vh] overflow-y-auto relative">
        ...
        {/* Header */}
        <div
          className={`mb-6 p-4 rounded-lg bg-gradient-to-r ${overallStatus.bgColor} border ${overallStatus.borderColor}`}
        >
        ...
        <div className="space-y-3 mb-6">
          {components.map((component) => {
            ...
            return (
              <Card
                key={component.name}
                className={`border-gray-200 ${displayStatus === "success" ? "bg-green-50" : displayStatus === "failed" ? "bg-red-50" : "bg-blue-50"}`}
              >
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-gray-600">
              {(() => {
                if (loadingPhase === "api") {
                  const apiComplete = Object.values(componentStatus).filter(
                    (status) => status === "success",
                  ).length;
                  return `${apiComplete} / ${components.length} API Calls Complete`;
                } else if (loadingPhase === "rendering") {
                  const rendered = Object.values(componentRenderingStatus).filter(
                    (status) => status === "complete",
                  ).length;
            ...
            <div
              ...
              style={{
                width: `${(() => {
                  if (loadingPhase === "api") {
                    return (
                      (Object.values(componentStatus).filter((status) => status === "success")
                        .length /
                        components.length) *
                      50
                    );
        ...
        {isRetrying && (
          <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-sm text-yellow-800">
              <strong>Retry in progress:</strong> Some components failed to load. Scout is
              automatically retrying failed components.
            </p>
          </div>
        )}
        ...
        {allSuccessful && (
          <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
            <p className="text-sm text-green-800">
              <strong>All set!</strong> All components have loaded successfully. The Scout screen
              will appear shortly.
            </p>
          </div>
        )}
