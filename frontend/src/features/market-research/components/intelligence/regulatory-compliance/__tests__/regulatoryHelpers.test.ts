...
import { getIconByName, getBadgeColor, deriveKeyDataPoints } from "@/features/market-research/components/intelligence/regulatory-compliance/regulatoryHelpers";
...
describe("regulatoryHelpers", () => {
  ...
  it("derives key data points from a keyUpdates array", () => {
    const pts = deriveKeyDataPoints(
      ...
      { euAiActDeadline: "Q1 2026", gdprCompliance: "x", potentialFines: "y", dataLocalization: "z" },
    );
  ...
  it("returns the hardcoded default set when keyUpdates is absent", () => {
    const pts = deriveKeyDataPoints(undefined, { euAiActDeadline: "Q1 2026", gdprCompliance: "x", potentialFines: "y", dataLocalization: "z" });
