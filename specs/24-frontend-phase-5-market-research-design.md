...
**Status:** Done — Phase 5 merged to master 2026-06-03 (design history: round 5) (rounds 1–4 reviews `docs/reviews/24-frontend-phase-5-market-research-design-spec-review-{1,2,3,4}.md` synthesized at `…-spec-synthesis-{1,2,3,4}.md` — round 4 refined the round-3 reconciliation; round 5 reconciles §4/§5 with shipped 5b reality after the 5c R1 escape hatch ×2 — see §9 deltas 7–8 / `docs/reviews/24c-frontend-phase-5c-R1-escape-hatch-findings.md`)
...
│   ├── useMarketResearch.ts          # 5b TanStack hooks: useResearchComponent + useRegenerateResearch
│   └── useMarketResearchData.ts      # 5c data-layer hook — the page's raw-fetch/useState/cascade/cache machinery extracted as-is (NO TanStack); shell calls it once + threads slices to tabs as props. The 5d–5h conversion target (§9 delta 8)
...
- ~~Rewire the (still-monolithic) page to consume the hooks; delete those raw `fetch` sites, the `CACHE_DURATION` cache, and the 68 localStorage refs that back it.~~ **DESCOPED during 5b execution (TD-FE-19 / §4 amendment below) — the page→hooks rewire is NOT done in 5b; it redistributes to 5c (structural relocation into the `useMarketResearchData()` hook — §5 / §9 delta 8; not into `IntelligenceTab`) + 5d–5h (per-section hook conversion, deleting each section's raw `fetch`/cache slice as it converts).** The page therefore still holds all 9 raw `fetch` sites + the `CACHE_DURATION` localStorage cache after 5b. `sessionStorage` used as primary state (if any survives) is left alone — that's not cache.
...
- **`useMarketResearchData()` as the conversion target (transition model — read this before planning 5c or 5d).** This hook is the linchpin connecting 5c's structural relocation to 5d–5h's TanStack conversion. It has three lives: **(1)** 5c creates it as a *verbatim* wrapper around the page's raw-`fetch`/`useState`/cascade/timestamp/localStorage-cache machinery — **no TanStack**, behavior byte-identical; **(2)** 5d–5h progressively replace its *internals* section-by-section, each section adopting the 5b TanStack hooks (`useResearchComponent`/`useRegenerateResearch`) and deleting its raw-`fetch`/cache slice (§6); **(3)** at phase close the hook delegates entirely to TanStack and zero raw `fetch`/`CACHE_DURATION` remain (24i confirms). It is **distinct from** the 5b hook `useMarketResearch` (which *is* the TanStack target it adopts inside itself) — same `hooks/` dir, different roles. Its consumers (the shell chrome + every tab) see a stable return surface across all three lives, so the 5d–5h internal swaps don't ripple outward.
...
**Done when** (each a gate):

- Page shell is a thin route-wire + tab router; it holds **no fetch-result/server `useState`** directly.
- `useMarketResearchData()` holds the raw-`fetch`/`useState`/cache machinery; the shell calls it once and threads slices to the tabs as props. (5c does **not** remove raw `fetch` from the feature — that completes across 5d–5h, confirmed at 24i.)
- Lead-stream (`analysis`) tab extracted to `src/components/market-research/lead-stream/`; it imports **no** feature hook; its data access lives inside the `ScoutLeadStream` it renders (the page fetches nothing for this tab — §4.1).
- Cross-tab/nav/Strategist coordination injected into the lead-stream unit as shell-owned callback props (one-way boundary).
- `SafeMarketIntelligenceTab` replaced by `<FeatureErrorBoundary>` with its prop-sanitization preserved (or its removal documented with rationale).
- Props-vs-context decision for the `scoutResearchContext`/`scoutMode` pair recorded.
- `journeys/04` behavioral parity + `npm run preflight` green.
...
- **No hard LOC cap** (master §6). The decomposition criterion is **single-responsibility, not a line count** — split a section by responsibility (one concern per file), not to hit a LOC target. Each 5d–5h plan proposes its file breakdown and justifies it on single-responsibility grounds; the plan reviewer assesses fitness (is each file one clear concern, holdable in context?). "Fits in agent context" is the downstream *benefit* of single-responsibility files, not the measurable target.
- Vitest + RTL tests for the section's hook + logic-bearing sub-components (§8).
- **Delete the page's raw `fetch`/cache machinery for this section as it converts.** Because 5b's page→hooks rewire was descoped (§4 amendment / TD-FE-19) and 5c only *relocates* the raw data flow into the `useMarketResearchData()` hook (§5 / §2.1 — the 5c R1 replan extracted it as a hook, not into `IntelligenceTab`), the actual removal of each section's raw `fetch` site + its slice of the `CACHE_DURATION`/localStorage cache + cascade/timestamp/edit-history handling happens **here**, when that section adopts `useResearchComponent`/`useRegenerateResearch` (migrating or consciously dropping each behavior per the section plan). This is the deferred half of the data-layer milestone (master Phase 5); 24i's zero-raw-`fetch` / zero-`CACHE_DURATION` gate depends on every section sub-phase doing it.
...
**Done when:** `index.ts` + `README.md` complete; handoff table is authoritative; `knip --strict` clean **and** zero `// DEAD CODE` annotations remain in the feature (the dead-file removal gate — knip itself does **not** flag unused *files* here, per the `knip.json` `entry` limitation above, so the 8 dead files are verified gone by annotation-absence, not by knip); preflight green.
...
- **Behavioral E2E** (`frontend/e2e/journeys/04-market-research-5-components.spec.ts`) + **Vitest/RTL** stay green **between every sub-phase** — the primary guard that decomposition preserved behavior. **There is no MR pixel-VR baseline (§9 delta 6 / TD-FE-17), so behavioral E2E + Vitest *is* the parity guard for this surface — not visual regression.** If the journey proves too thin for a given sub-phase, extend it (existing tests must stay green — master §2.3).
...
2. **Status table (§4) — amendment.** Mark Phase 5 → in progress now, → done (with date) at final merge.  **(Done 2026-06-03 at the 24i merge.)**
...
4. **Phase-number reconciliation.** Master §4 overview (signals 8 · scout 9 · settings 10) conflicts with the Phase 4b `features/README.md` naming map (signals 6 · scout 8 · settings 11). Pre-existing drift, surfaced (not caused) by Phase 5. Recommend reconciling the master plan to one source of truth; until then handoffs reference target features by name. Log as a master-plan delta (or `TD-FE-<n>` if not resolved at Phase 5 merge).  **(Unresolved at the Phase 5 merge — logged as TD-FE-32; handoffs reference target features by name.)**
...
9. **5c shipped — hook-first decomposition complete (recorded 2026-06-01).** Plan 24c (hook-first) implemented across 6 structural tasks, each two-stage-reviewed (spec-compliance + code-quality), all green at final `npm run preflight` (typecheck, lint, format, Vitest, build, bundle-advisory, all 14 e2e journeys incl. `journeys/04`, `knip --strict`). Outcome: `MarketResearchPage.tsx` **7,013 → ~1,100 LOC** (a thin route-wire + tab router + the shell-owned scout cross-tab pair + 3 analysis handlers + shell chrome). The data layer moved verbatim into **`useMarketResearchData()`** (no TanStack rewire — 5d–5h convert its internals). Tabs: `IntelligenceTab` (genuine, data via props), legacy `LeadStreamTab` (HANDOFF→customers, shell-owned callback props, one-way boundary), `TrendsTab` (thin router over leaving Scout-chat; out-of-band block lifted, empty placeholder removed). `SafeMarketIntelligenceTab` + `MarketIntelligenceTab` deleted; sanitization preserved as the tested pure `sanitizeIntelligenceProps`; `<FeatureErrorBoundary featureName="Market Intelligence">` replaces the generic boundary. **Decisions recorded:** (a) `scoutResearchContext`/`scoutMode` hoisted as **props, not context** (2 shallow consumers — `MarketResearchContext` NOT created); (b) `activeTab` stays `location.pathname`-derived (no `useParams`); (c) filter split — `leadStreamFilters` is analysis-local (moved into `LeadStreamTab`), `opportunityFilter` is cross-tab (kept in the hook, passed as a prop) — refines §5's "both internal" presumption against live code. **Deferred:** TD-FE-20 (no e2e parity coverage for the `trends`/`analysis` tabs — pre-existing, advisory, revisit Phase 7); the two pre-existing commented-out JSX blocks in the shell (Scout header, Settings/History/Refresh) — removed post-review (impl-review F5, commit removing 264 dead-comment lines; shell 1,098→834 LOC). 5d–5h read from the now-isolated `useMarketResearchData` + `IntelligenceTab`, converting raw `fetch`→5b hooks section-by-section (each deleting its page-origin `fetch`/cache slice — §6); 24i confirms zero raw `fetch` + zero `CACHE_DURATION`.
10. **5h shipped — `MarketSizeSection` decomposed + `MarketIntelligenceTabProps` deleted (recorded 2026-06-03).** Plan 24h implemented: `MarketSizeSection` (≈1,660 LOC monolith) is now a thin container (≈870 LOC) under `components/intelligence/market-size/` over single-purpose sub-components (`MarketSizeHeader`, `ExecutiveSummary`, `KeyMetrics`, `StrategicRecommendations`, `MarketDrivers`, `MarketSizeBySegment`, `GrowthProjections`, `MarketEntry`, `ExportOptions`, `states`) + pure chart-data helpers (`marketSize.ts`: `segmentsToPieData`/`projectionsToLineData`) + the section hook `useMarketSize` over the 5b data layer (`useResearchComponent`/`useRegenerateResearch`), all with co-located Vitest/RTL tests. As the **last** consuming section, 5h **deleted `MarketIntelligenceTabProps.ts`** (the single-export umbrella interface, no remaining importer) — the composition layer (`MarketIntelligenceSections.tsx`) now defines its own `MarketIntelligenceSectionsProps` (the old contents **minus the 9 market-size data fields**, every other field verbatim); `sanitizeIntelligenceProps`/`IntelligenceTab` repointed; guarded by an empty `grep` + `knip --strict`. The prop-drilling interface (§1.2/§2.3) is fully retired. **Divergences from the plan text (code is authoritative):** (a) the plan's assumed local export helpers (Blob/`createObjectURL`/`window.print`/clipboard) and a Sources modal **do not exist** — export is three parent callbacks (`onExportPDF`/`onSaveToWorkspace`/`onGenerateShareableLink`), so no `exportMarketSize.ts` pure-helper file was created and `ExportOptions` is a de-dup of the two button rows; (b) no `getDisplayX` prop-vs-local switch — replaced by a single `view` object sourcing from `useMarketSize.data` (`?? default` only); convention followed = 5g (industry-trends). **Two documented deferrals (done-when #5 fallback):** (i) the page-level `useMarketResearchData()` **market-size slice was NOT removed** — `marketData`/`fetchMarketSizeData` is the **cascade ROOT** (priority-1 producer feeding all four downstream sections via `previousContext`); removing it section-by-section would hollow the cascade (abort-criterion-3), so it is retained and logged as **TD-FE-30**, to be retired wholesale with the cascade at 5i/24i's zero-raw-`fetch` gate (same posture as TD-FE-19/28, escalated as the root). (ii) the section's `/api/ask` edit-save (`handleSave` + `setUserLocalStorage`) is a different endpoint outside 5b's `market-research` layer — kept this phase and logged as **TD-FE-31** (mirrors TD-FE-21 for market-entry). Visual guard = behavioral E2E (`journeys/04`) + Vitest/RTL + `npm run preflight`; no market-research pixel VR.
11. **24i closed Phase 5 with the zero-raw-`fetch` gate RELAXED to advisory — data-layer migration carried past Phase 5 (recorded 2026-06-03).** Spec §11 item 3 and deltas 7–10 anticipated 24i would retire the page-level data cascade wholesale (TD-FE-19/21/27/28/30/31) so that zero raw `fetch` + zero `CACHE_DURATION` remained in the feature at phase close. At the 24i close the feature still carries the ~6k-LOC `useMarketResearchData.ts` (raw `fetch` + editable-state/cascade + localStorage cache) plus per-section `/api/ask` writes — i.e. the **whole** data-layer migration is still pending — the cascade ROOT (TD-FE-30) is the mechanical blocker that kept the other slices (TD-FE-19/21/27/28/31, each independently gated on 24i) from being retired, not an isolated tail item the plan expected to leave behind. **Decision (CTO, pre-launch advisory-over-hard-fail posture, 0 live users):** the zero-raw-`fetch` gate is **relaxed from hard to advisory** for the Phase-5 close. What is **done**: the structural decomposition — page shell + tab router + five decomposed sections (5a–5h) + the 5b memory-only TanStack layer (`useResearchComponent`/`useRegenerateResearch`, the section-display source) + `index.ts` public surface + `README` + the 8-file dead-code sweep. What is **deferred** (tracked, not dropped): the page-level cascade/edit-history migration off raw `fetch`/localStorage, carried forward as TD-FE-19/21/27/28/30/31 — triggers updated to point past Phase 5 to the Phase 7 lead-stream-era mutation pass and the Phase 13 audit. **Spec §11 item 3 is therefore PARTIALLY met at close** (the memory-only TanStack layer exists and backs section display; the legacy page cascade is retained, not removed). No behavior change at close — this is a scope/record decision, not a code change. The phase-13 boundary (delta 5) widens accordingly: Phase 13 (or the earlier claiming phases) inherits the first-time data-layer migration of this cascade, not merely a verification pass.
...
3. Data layer is TanStack Query (memory-only); no raw `fetch` or localStorage cache in the feature — **completed across 5d–5h** (5b built the layer; 5c relocates the still-raw page data into the `useMarketResearchData()` hook — §5 / §9 delta 8, not `IntelligenceTab`; each section converts to hooks as it decomposes, its cascade/timestamp/edit-history slice migrating or being consciously dropped per its section plan — §4 amendment / TD-FE-19). `24i` confirms zero raw `fetch` + zero `CACHE_DURATION` remain at phase close. **— AMENDED AT CLOSE (see §9 delta 11): this item is only PARTIALLY met. The page cascade migration was deferred, so `useMarketResearchData.ts` still holds raw `fetch` + `CACHE_DURATION`; 24i relaxed the gate from hard to advisory (CTO, pre-launch posture) rather than retiring it. The 5b memory-only TanStack layer backs section display; the legacy page cascade is carried forward as TD-FE-19/21/27/28/30/31.**
4. Routes resolve to the feature; **URLs unchanged**.
5. Vitest + RTL coverage for the feature's logic-bearing units; `journeys/04` E2E green (visual parity via behavioral E2E; **no MR pixel VR** — §9 delta 6); `knip --strict` clean **and** zero `// DEAD CODE` annotations remain (knip does not flag dead *files* here — §7); `npm run preflight` green.
...
- **R4 — Visual diffs from incidental Tailwind/markup changes.** *Mitigation:* parity is guarded by behavioral E2E (`journeys/04`) + Vitest/RTL — **there is no MR pixel-VR baseline (§9 delta 6 / TD-FE-17)**; class consolidation only when behaviorally neutral. (The "visual regression at 2%" framing from rounds 1–4 is superseded by delta 6 for the market-research surface.)
...
- ~~The exact route URL(s) + `:tab` segments as currently configured in `App.tsx` (`24a`/`24c` confirm; they stay frozen).~~ **RESOLVED in 5a** (§9 delta 6): `/your-ai-team/scout/:tab`, segments `marketintelligence`/`leadstream`/`chatwithscout` (↔ internal keys intelligence/analysis/trends), with `/market-research` + `/your-ai-team/scout` redirects — frozen.
