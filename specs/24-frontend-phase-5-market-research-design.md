...
|---|---|
| Page | `src/pages/MarketResearch.tsx` — **7,013 LOC**, a single `React.memo` component (**~70 `useState`** — 88 `useState(` tokens, ~70 distinct hooks — `24c` Task-0 count; the round-1 "49" was an undercount, **24 `useEffect`**, `useCallback`/`useRef`). Post-Phase-1 (was 14,956). `MarketResearch_clean.tsx` duplicate **verified absent** (a Phase 1 target — master §4 Phase 5). The `useState` set is dominated by **per-section ephemeral editing state** (editing/expanded/hasEdits/deletedSections/editHistory/customMessage/showScoutChat/loading/error ×5 sections) plus six **editable** data states (see §4 note / TD-FE-19) — not 70 independent shared variables. |
...
**Mission:** break the 7k-LOC page into a thin shell + tab containers and rehome the ~70-`useState` soup. **5c is structural-only** — it relocates JSX + state into tab containers; it does **not** wire the page to the 5b hooks (that rewire was descoped from 5b and distributes into 5d–5h — §4 amendment / TD-FE-19).
...
- **State rehoming (structural):** the page's existing **raw-`fetch` + `useState` server-data flow moves into `IntelligenceTab` UNCHANGED** — 5c does not convert it to hooks (the six data states are editable/cascade/timestamp state, not plain server cache — §4 amendment; per-section hook conversion is 5d–5h). `activeTab` → stays URL-derived (`getActiveTabFromPath` reads `location.pathname`; **not** `useParams`); ephemeral state (open dialogs, input drafts, interim loading, the per-section editing flags) → local `useState` in the tab container / section that owns it; genuinely-shared coordination → `MarketResearchContext` **or** shell-lifted props per the criteria below. **Search/filter state** follows the URL-vs-local constraint below; its exact treatment is a `24c` decision (§13), not settled here.
- **Cross-tab shared state (the R1 finding — corrects the "no context expected" presumption):** the `analysis`-tab handlers write `scoutResearchContext`/`scoutMode`, which the `trends`-tab chat reads. That pair is shared across ≥2 tabs, is not URL-derivable, and is not server state — so it **meets all three context criteria** and is a genuine candidate for `MarketResearchContext` (or shell-`useState` lifted and passed as props to both `LeadStreamTab` and `TrendsTab`). 5c chooses context-vs-props per the criteria; the point is that **5c is not expected to land at zero shared state** — this is the cross-tab coupling R1 flagged, handled in-plan rather than via revert.
...
- Replace the bespoke `SafeMarketIntelligenceTab` wrapper with `<FeatureErrorBoundary>`; drop any unused `Safe*` wrappers. **Note (R1 finding):** `SafeMarketIntelligenceTab` is **not just an error wrapper** — it performs real **prop-sanitization** before rendering `<MarketIntelligenceTab>` (and wraps in the generic `ErrorBoundary` from `@/components/common/ErrorBoundary`, not an MR-specific one). 5c must **preserve that sanitization** (move it into `IntelligenceTab`) when swapping the boundary, or consciously document its removal with rationale in the plan/commit — a blind swap silently drops behavior. `MarketIntelligenceTab` is deletable once Safe is gone (its only importer is Safe; keep `MarketIntelligenceSections` + `MarketIntelligenceTabProps.ts`, which 5d–5h consume/retire).
...
**Done when:** the **page shell** is a thin route-wire + tab router with **no fetch-result/server state held in the shell's own `useState`** (the raw-`fetch`/cache machinery moved into `IntelligenceTab`, which still carries it until its sections convert in 5d–5h — **5c does not remove raw `fetch` from the feature**; that completes across 5d–5h, confirmed at 24i); the lead-stream tab is an annotated unit in `src/components/market-research/lead-stream/` carrying its own raw `fetch`, importing no feature hook (cross-tab/nav coordination injected as shell-owned callback props); `SafeMarketIntelligenceTab` is replaced by `<FeatureErrorBoundary>` with its prop-sanitization preserved (or its removal documented); the `MarketResearchContext`-vs-shell-props decision for `scoutResearchContext`/`scoutMode` is recorded; parity (`journeys/04`) + preflight green.
