...
> **Phase 11 deltas (recorded 2026-06-06, frozen-record convention — does not rewrite the intent above).** Legacy dirs drained into `src/shared/*` and `features/*`: `hooks/usePageTitle` → `shared/hooks/`; `lib/jwt.ts` + `hooks/useAuth.ts` → `shared/auth/` (useAuth renamed to `useAuthToken` to avoid collision with Firebase `useAuth`); `lib/api.ts` → `shared/api/transport.ts`; `lib/cacheUtils`, `lib/sanitizeAnswerText`, `lib/leadData` → `shared/lib/`; `types/escape-hatches` → `shared/types/`; `styles/` → `shared/styles/` (`App.css` deleted as dead); `components/common/ErrorBoundary` → `features/customers/components/`; `utils/leadStreamChatContext` → `features/market-research/lib/` (sole-consumer); `components/market-research/` fully drained (ScoutLeadStream + lead-stream cluster → `features/market-research/`; EditDropdownMenu → `features/customers/`; leadData promoted to `shared/lib/`); `components/ui/use-toast`, `components/ui/use-mobile`, `components/ui/utils` (cn) absorbed into `components/ui/`; `PWAInstallPrompt` → `features/shell/`. All legacy-dir imports repointed; ADR-0005 records the Phase 11 decisions. TD-FE-54/57/62/63 resolved. **Erratum:** Spec 31 §3's summary tree lists `use-toast` under `shared/hooks/`, but §5.1 (authoritative) places it in `components/ui/use-toast.ts`; the Phase 11 implementation followed §5.1. Authority: Spec 31 / plan 31.
...
12. **`src/styles/`** — stays at root, moves to `src/shared/styles/`, or distributes into features? → Phase 11 spec — **RESOLVED (Phase 11, 2026-06-05):** relocated to `src/shared/styles/`; `App.css` deleted as dead (no remaining importers).
...
14. **Phase 11 standalone vs absorbed** — with shell extraction moved to Phase 4, the narrowed Phase 11 (shared utility extraction only) could fold into the Phase 13 cleanup audit. → Decided based on what Phases 5–10 surface as shared — **RESOLVED (Phase 11, 2026-06-05):** executed as a standalone phase draining legacy dirs (`hooks/`, `lib/`, `utils/`, `contexts/`, `styles/`, `components/common/`, `components/market-research/`) into `src/shared/*` and `features/*`.
...
16. **Lint rule for `index.ts`-only enforcement — RESOLVED (Phase 4a, 2026-05-29; implemented Phase 6 stage 1b, 2026-06-03):** tooling is `eslint-plugin-import-x` + `eslint-import-resolver-typescript`. 4a enforces the cross-zone boundaries (`import-x/no-restricted-paths`) and `import-x/no-cycle`; the `index.ts`-only rule (`import-x/no-internal-modules`) was **deferred** (TD-FE-15) after a spike. Phase 6 stage 1b lands the rule in `forbid` form — `forbid: ["@/features/*/!(index)", "@/features/*/!(index)/**"]` — in `frontend/eslint.config.js`; the `!(index)` extglob excludes the resolver-appended `/index` terminal so bare index imports are not flagged. TD-FE-15 resolved.
