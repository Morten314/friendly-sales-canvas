...
| `src/lib/marketScoreDescriptions.ts` | The exported `getDescriptionTextForColumn(response, reportColumnKey)` returns the description text for a report-column key (e.g. `"market-size"`) via the `REPORT_KEY_TO_DESCRIPTION_LABEL` map; case-insensitive + whitespace-trim fallback when the canonical key isn't found verbatim in `response.descriptions`. Cover known keys, unknown keys, missing/undefined response. | Static lookup table — fast. (Earlier draft of this row referred to "score band descriptions"; the function is a column-key lookup. Corrected pre-merge.) |
...
| `src/lib/utils.ts` | Two exports: `cn()` (shadcn's classnames helper — covers conditional classes, conflicting Tailwind classes resolved by `twMerge`, undefined/null inputs) AND `sanitizeAnswerText(text)` (markdown/symbol stripper used in agent-answer display — covers each regex-replace branch: emphasis markers, headers, code fences, em/en-dashes, smart quotes, emoji surrogates, whitespace collapse). | Two-function file: shadcn template plus a domain helper. (Earlier draft of this row mentioned only `cn`; both are pure-survivor utilities and both ship with characterization.) |
| `src/lib/rateLimitManager.ts` | The 30-req/min boundary holds; queues when cap is hit; releases as the rolling window slides; behavior under concurrent enqueues. | **Stateful** singleton — not a pure function. Use `vi.useFakeTimers()` exclusively; reset module state between tests via `vi.resetModules()` or a per-test re-import. The 30 req/min cap is a frozen interface per master §2.3; the in-code comment "increased limit for faster processing" is the normative value (earlier drafts of this row stated 4 req/min — the default was bumped before 0b implementation and this row was reconciled pre-merge). |
...
| R0b-2 | Characterization tests for `rateLimitManager` are flaky due to timing. | Use `vi.useFakeTimers()` exclusively. No `setTimeout`-based real-time waits. The 30 req/min cap is a frozen interface; the test is deterministic against fake timers. |
