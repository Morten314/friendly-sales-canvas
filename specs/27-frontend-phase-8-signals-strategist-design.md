...
3. **`StrategistWorkspace` (963 LOC) is live, and strategist *does* have a backend call** (contra the master plan's "Strategist has no backend"). Nothing in `src/` writes `sessionStorage.strategistContext` (verified by textual grep of `strategistContext` across `src/` — only the two reads in `Deals.tsx` exist; a separate grep of navigate-to-strategist call sites found none setting it). But `Deals.tsx` does **not** gate the Workspace on that context: it renders **three** tabs (`workspace` / `recommendations` / `leadstream`), each its own `TabsContent`, and the default `workspace` tab renders `<StrategistWorkspace leads={context?.leads ?? []} …/>` **unconditionally**. The Sidebar and `LeadsTable` both navigate to `/strategist/workspace`, so the Workspace **is reachable** — it simply renders with empty leads (always, today). Its `GET /chat/` is therefore a **live** backend call; only the lead-*prepopulation* path (a populated `strategistContext`) is unexercised. **Resolution:** relocate as-is (§6) — its 963-LOC decomposition and its `GET /chat/` TanStack migration are **deferred to Phase 13** (TD-FE-47), not because it's dead but because decomposing/migrating a 963-LOC component would over-scope this extraction phase; the unexercised prepopulation path is preserved verbatim for parity.
...
| File | LOC | Role |
|---|---|---|
| `src/pages/Deals.tsx` | 91 | Strategist route page. Reads (+ clears) `sessionStorage.strategistContext` (pre-populates leads/opportunity/icp when set — currently never set). **Three** tabs, each its own `TabsContent`: `workspace` → `<StrategistWorkspace leads={context?.leads ?? []} …/>` (default), `recommendations` → `<StrategistRecommendations/>`, `leadstream` → `<StrategistLeadStream/>`. **Only consumer** of all three strategist components. |
| `src/components/market-research/StrategistWorkspace.tsx` | 963 | **Live** — rendered on Deals' default `workspace` tab. Raw `fetch` `GET ${BACKEND_BASE_URL}/chat/?question=` (its only network call). File-header `// HANDOFF → strategist` annotation. |
| `src/components/strategist/StrategistLeadStream.tsx` | 309 | Reached (`leadstream` tab). No fetch, no storage. |
| `src/components/strategist/StrategistRecommendations.tsx` | 210 | Reached (`recommendations` tab). No fetch, no storage. |

Full strategist surface ≈ 1,573 LOC (Deals 91 + Workspace 963 + LeadStream 309 + Recommendations 210), **all reachable** via the three tabs. The lone strategist backend call (`StrategistWorkspace`'s `GET /chat/`) is therefore **live**; Phase 8 relocates Workspace as-is and defers its decomposition + TanStack migration to Phase 13 (TD-FE-47).
...
- **No `StrategistWorkspace` decomposition or fetch migration.** It is live but large (963 LOC); relocated as-is, with its decomposition + the `GET /chat/` TanStack migration deferred to Phase 13 (TD-FE-47). The unexercised lead-prepopulation path (no `strategistContext` writer) is preserved verbatim for parity, not removed.
...
**Strategist data layer is deferred, not absent.** `StrategistWorkspace` is live and makes a raw `GET /chat/` call, but Phase 8 relocates it as-is and **defers** its TanStack migration + decomposition to Phase 13 (TD-FE-47) rather than decomposing a 963-LOC component in an extraction phase. `Recommendations`/`LeadStream`/`StrategistPage` have no fetch. So 8d does no TanStack work — by deferral, not because the surface is call-free.
...
- **`Deals.tsx` → `features/strategist/pages/StrategistPage.tsx`** (rename; the file name `Deals` is vestigial — §1.2.4). Its `sessionStorage.strategistContext` read/clear and the **three-tab** structure (`workspace`/`recommendations`/`leadstream` → Workspace/Recommendations/LeadStream) are preserved **verbatim** for parity; `StrategistWorkspace` receives `leads={context?.leads ?? []}` exactly as today (the prepopulation path is unexercised but preserved). The `StrategistContext` interface moves to `features/strategist/types.ts`.
...
- **`StrategistWorkspace` → `features/strategist/components/` AS-IS** (relocate as-is, per the chosen disposition). It is **live** (rendered on the default `workspace` tab) but a 963-LOC component with a raw `GET /chat/` call; Phase 8 relocates it unchanged and **defers** both its decomposition and its `GET /chat/` TanStack migration to Phase 13 (TD-FE-47) — doing either now would over-scope this extraction phase (the "Full" decomposition choice applies to the Signals page). Its now-obsolete `// HANDOFF → strategist` file-header annotation (which says the file "stays here until the strategist feature phase relocates" it) is **removed** — leaving it would be actively false post-relocation; removal, not a prose rewrite, avoids needless churn in an otherwise minimally-touched file. The `leads`/`opportunity`/`icp` props still flow from `StrategistPage`'s `context?.leads ?? []` (prepopulation path unexercised but preserved).
...
| R5 | Mis-classifying `StrategistWorkspace` leads to wrong handling (it was initially mis-read as dead; round-2 corrected). | It is **live** (default `workspace` tab, empty leads); only the `strategistContext` lead-prepopulation path is unexercised (no writer in `src/`). Relocate-as-is preserves both the live render and the unexercised path verbatim; decomposition + `GET /chat/` migration deferred to Phase 13 (TD-FE-47). |
...
- **TD-FE-47** — `StrategistWorkspace` (963 LOC) relocated as-is: a **live** component (default `workspace` tab) carrying a raw `GET /chat/` fetch, neither decomposed nor TanStack-migrated this phase (would over-scope the extraction). Phase 13 decomposes it + migrates the fetch. Separately, its `sessionStorage.strategistContext` lead-prepopulation path has **no writer** in `src/` (renders with empty leads); Phase 13 decides whether to wire it up or remove it.
