...
- `src/shared/auth/` — `AuthContext.tsx` (`AuthProvider`, `useAuth` → `login`, `signup`,
  `fetchOrgId`, `currentUser`), `index.ts`.
...
- Sources to move: `pages/{Login,TenantSelection,Settings}.tsx`, `pages/{useLogin,useTenants}.ts`,
  `components/settings/{CompanyProfile,UserProfile,AgentProfile}.tsx`,
  `components/settings/useCompanyProfile.ts`, and their `__tests__`.
...
| `features/settings/` | `pages/SettingsPage.tsx`, `components/{CompanyProfile,UserProfile,AgentProfile}.tsx`, `hooks/useCompanyProfile.ts` (`useCompanyProfile`+`useSaveCompanyProfile`) | `pages/Settings.tsx` (291), `components/settings/*`, `useCompanyProfile.ts` (56) |
...
src/features/
  ...
  settings/
    ...
    hooks/useCompanyProfile.ts   # useCompanyProfile, useSaveCompanyProfile
    routes.tsx                   # "/settings" (ProtectedRoute requireTenant)
    index.ts
    README.md
    __tests__/{CompanyProfile,useCompanyProfile}.test.tsx
...
- `settings/` → `shared/auth` (`useAuth`), `shared/api` (`apiGet`/`apiPost`, contracts,
  `qk.companyProfile`), `components/ui`.
...
`features/auth/hooks/useLogin` → `@/shared/auth/firebase`. It need not be re-exported from
`shared/auth/index.ts` (internal to the foundation). `lib/jwt.ts` stays put (§2.2).
...
until Phase 9 relocates it into `features/scout/`. `AgentProfile` is a generic settings form (agent
name / tasks / instructions); it is not scout-coupled and belongs to settings.
...
3. **settings/:** relocate Settings + {CompanyProfile,UserProfile,AgentProfile} + useCompanyProfile;
   `routes.tsx`; `index.ts`; tests; verify.
...
- Relocated test files move with their code (import-path updates only). The shared
  `http.get("/api/profile/company")` MSW handler stays in `src/test/msw/handlers.ts`.
...
- **Only expected conflict: `App.tsx`.** Phase 8 removed `import Signals` (which sits between
  `import Settings` and `import TenantSelection`); Phase 10 removes those two adjacent imports →
  a ~3-line textual conflict in the import cluster. Mechanical (keep both removals), resolved once at
  the merge gate. The `<Routes>` removals are in different regions (no overlap). Batching Phase 10's
  `App.tsx` edits into one late commit (§7 stage 4) keeps the window minimal.
...
- `features/{auth,tenant,settings}/` each have page + hooks + `routes.tsx` + `index.ts` + `README.md`
  ...
  (+ `useCompanyProfile.ts`) no longer exist at their old paths.
- `firebase.ts` lives in `shared/auth/`; both importers updated.
