...
1. **Audit (Stage 1).** Read every Python file under `backend/app/`. For each file, categorize findings using §4's opportunity taxonomy. Tag each finding `execute` / `investigate` / `design-discussion`. Commit the scorecard as a frozen-intent artifact at `docs/audits/2026-05-26-backend-loc-docstring-audit-phase-l.md`.
...
2. **Investigation (Stage 2).** Each `investigate` finding gets a focused per-site analysis. Read every call site fully, write a behavior-preservation strategy, conclude:
   ...
   - **Soft cap:** any single investigation that's still inconclusive after ~60 minutes of focused reading + targeted testing is deferred. The phase doesn't stall on one stubborn finding.
...
- **The audit scorecard** at `docs/audits/2026-05-26-backend-loc-docstring-audit-phase-l.md` covering all 91 files with a verdict each.
...
File: `docs/audits/2026-05-26-backend-loc-docstring-audit-phase-l.md`. Frozen-intent artifact, committed once at end of Stage 1, updated once at end of Stage 2.
...
- Once committed, the scorecard is a frozen historical record — same convention as specs/plans (per CLAUDE.md "Specs and plans are a frozen record of intent, not current truth").
...
| K4 | Extract `fetch_company_profile` to `services/_neo4j_helpers.py`; replace 7 call sites across 5 files | `services/_neo4j_helpers.py`, `services/customer_profile/orchestrator.py` (3 sites), `services/market_scoring/persistence.py`, `services/market_research/orchestrator.py`, `services/icp/orchestrator.py`, `services/signals/ask.py` (2 sites) | 7 | -50 | ✓ Same Cypher query `MATCH (c:CompanyProfile {org_id: $org_id}) RETURN c LIMIT 1` (alias variant in `signals/ask.py` uses `p:`/`RETURN p`, semantically equivalent for the fetch use case). |
| K5 | Extract `_update_run(run_coll, run_id, **fields)` helper; replace 10 call sites | `services/market_scoring/scoring.py` | 6 | -50 | ✓ 10 `run_coll.update_one(...)` calls at lines 48, 55, 69, 83, 97, 112, 162, 173, 192, 208. |
| K6 | Extract `_get_file_collection(mongo)` helper; replace 11 call sites | `services/data_sources/persistence.py`, `services/data_sources/pipeline.py` | 5 | -22 | ✓ 11 instances of `db = mongo["File_Processing"]; collection = db["file_status"]` across the two files. |
| K7 | TD-009 docstring drift sweep — remove Phase/commit/extracted-from/final-form/renamed references | ~12 files across `services/`, package `__init__.py` files | 2 | ~-90 | ✓ 25 grep matches across `backend/app/` for the targeted patterns. |

**Known-wins subtotal:** ~430 LOC reduction. Plus any audit-surfaced additions and promoted investigation findings.
...
- **K2**: implementation writes a temporary script that imports both pre-refactor constants (via stash apply of a baseline) and the post-refactor base+overlay assembly. Asserts `assembled == original` byte-for-byte for `Cypher_gen_prompt`, `Cypher_gen_prompt2`, `qa_prompt_template`, `qa_prompt_template2`. Script is discarded before commit; the assertion's success is documented in the commit body.
- **K3**: parametrized pytest (added in the commit, kept thereafter) calls each component's `template.format(company_profile_json=<fixture>)` and asserts byte-equality against a checked-in fixture per component. Fixtures are generated from the pre-refactor functions on a baseline branch.
- **K4**: helper signature `fetch_company_profile(driver, org_id: str | None) -> dict | None`. Returns either a `dict` (the first record's values) or `None`. The 7 call sites already follow this pattern; the helper just centralizes the Cypher + alias-handling. Existing tests for the 5 consuming services continue to pass.
...
- **K7**: prose-only edits inside docstrings. Replacements are structural-only — what the module exports, what its public API is. No version/commit/phase references in the new text. If a structural-only replacement isn't possible, the docstring becomes minimal or is removed.
...
3. **Behavior-preservation evidence** specific to the task (assertion script for K2; parametrized test for K3; existing-tests-pass for K4; pyflakes for K1; trivial-helper-equivalence for K5/K6; prose-only for K7).
...
| R7 | Investigation phase extends the round significantly | Accepted, with soft cap: any single investigation still inconclusive after ~60 minutes is deferred and documented. The phase doesn't stall on one stubborn finding. |
...
1. **Audit scorecard committed** at `docs/audits/2026-05-26-backend-loc-docstring-audit-phase-l.md` with all 91 files of `backend/app/` represented, each with a verdict.
...
4. **TD-009 closure:** `grep -rnE 'Phase [A-Z]|commit [0-9]+/[0-9]+|extracted from .* in Phase|final form|Renamed.*in Phase' backend/app/` returns 0 matches.
5. **Full pytest suite passes** on the final commit.
6. **No new pyflakes warnings** introduced by Phase L.
7. **Impl review verdict: clean** (or, if any actionable findings, fix-then-clean).
8. **Branch fast-forward-merged into master and pushed.**
