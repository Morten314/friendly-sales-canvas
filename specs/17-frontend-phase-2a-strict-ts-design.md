...
- If escape hatches were needed, `src/lib/types/escape-hatches.ts` holds them with documented justification and call-site references. Soft cap 5 (auto-raise to 10 with TD-FE registration); hard cap 10 triggers a user checkpoint (see §3 Step 3).
...
| Current `tsconfig.app.json` (5 explicit flags) | `strict: false`, `noImplicitAny: false`, `noUnusedLocals: false`, `noUnusedParameters: false`, `noFallthroughCasesInSwitch: false`. (`strictNullChecks` is not explicitly listed in the app config — its effective value comes from `strict: false` and the composite root's override below.) |
...
- Create `src/lib/types/escape-hatches.ts` **only if needed**. Soft cap 5 entries (auto-raises to 10 with a TD-FE entry capturing the pattern); hard cap 10 triggers a user checkpoint with three options — raise further, defer remaining hatch sites as TD-FE, or abort per Spec 14 §5.7 (see §3 Step 3 for the policy). Each entry requires (a) a comment naming the call site and (b) a one-line justification for why proper typing is unreasonable at this phase. **The interim location `src/lib/types/escape-hatches.ts` deviates from Spec 14 §4 Phase 2a's `src/shared/types/escape-hatches.ts` only because `src/shared/` doesn't exist until Phase 4** — Phase 4 relocates the file to the master-plan-specified path when it creates `src/shared/`.
...
**Escape-hatches policy (two-tier cap with user checkpoint at the hard cap):**

The 5/10 thresholds are calibrated against the 83 TS7006 (implicit-any) baseline — 5 ≈ 6% of TS7006 (a reasonable buffer for genuinely hard cases); 10 ≈ 12% (the structural signal that the spec under-scoped semantic work). If Step 0 re-baselines the implicit-any count materially above 83, the plan author proportionally re-scales the thresholds before Wave B starts.
...
- **Soft cap 5.** Default: `src/lib/types/escape-hatches.ts` is empty or absent. Each entry is added only when proper typing is genuinely unreasonable for this phase. Format:
  ...
  The `// TODO(phase-13):` comment is mandatory on every entry — it gives Phase 13's audit a greppable marker. The `EscapeHatch*` type prefix is also mandatory (deliberately ugly, signals temporariness via namespace).
- **Soft cap reached (5th entry added):** continue without halting. The 5th entry's commit body includes a `TD-FE-<n>` registration line (numbering continues from TD-FE-8 or wherever the register stands). The TD-FE entry captures: which 5 sites used escape hatches, the pattern they share (if any), and the trigger (typed-data missing? generic refactor needed?). The cap auto-raises to 10 for the remainder of the phase.
- **Entries 6–10:** follow the same per-entry justification rules (call-site reference + one-line reason + TODO comment) but do not require additional `TD-FE-<n>` registrations — the 5th-entry TD-FE already captured the pattern.
- **Hard cap 10 → user checkpoint, not auto-abort.** If the 10th entry is reached, Phase 2a halts and asks the human orchestrator to choose from three options:
  - (a) **Raise the cap further** (e.g., to 15 or 20). The spec is amended at merge to record the new cap and the rationale.
  - (b) **Defer the remaining hatch sites as TD-FE entries** without escape-hatch typing. The unhatched sites' strict errors get either a proper type fix (back to default posture) or a per-site `TD-FE-<n>` carrying the deferral.
  - (c) **Abort the phase per Spec 14 §5.7.** Revert the branch; the TD-FE from the soft-cap raise captures the discovery; a revised spec is written before retry.

  The user-checkpoint design matches Spec 14 §5.6 ("Adjudicate conflicting agent rounds when synthesize step can't reconcile") and Spec 14 §4 Phase 2a's intent that Phase 13's audit re-evaluates remaining entries. Auto-abort was rejected as disproportionate.
...
- `src/lib/types/escape-hatches.ts` absent OR ≤10 entries (or whatever cap the user-checkpoint amended to, if Wave B hit the hard cap), each with the required `// TODO(phase-13):` comment and call-site reference. If >5, a `TD-FE-<n>` entry exists capturing the soft-cap-raise trigger.
...
4. **Escape-hatches:** count, location (`src/lib/types/escape-hatches.ts` if present), list of entries with justifications and call-site references, OR `none created`. If the hard cap triggered a user checkpoint, the resolution (raise / defer / abort) is recorded.
...
5. `src/lib/types/escape-hatches.ts` is absent OR contains **≤10 entries** (or up to whatever cap a user-checkpoint amended the spec to, if the hard cap triggered during Wave B). Each entry has the mandatory `// TODO(phase-13):` comment, a call-site reference, and a one-line justification. If the count is >5, a `TD-FE-<n>` entry captures the soft-cap-raise trigger.
...
| R2 | A Wave C semantic error reveals a structural problem requiring out-of-scope refactor (e.g., a type-narrowing fix that cascades across 10 files). | Escape-hatches two-tier cap with user-checkpoint at the hard cap (§3 Step 3). Soft cap 5 → continue with TD-FE registration capturing the pattern; hard cap 10 → user picks raise / defer-as-TD-FE / abort. Auto-abort was rejected as disproportionate to the scope of "10 hard cases out of 461 errors." The cap-ladder + checkpoint preserves the structural signal that the spec under-scoped semantic work while keeping the decision with the human orchestrator (Spec 14 §5.6). |
...
5. **Diff-size target enforcement.** §3 Step 5 sets ~1,000 lines reviewable surface as a soft target. Plan confirms whether to track it explicitly during impl-review or treat as informational only.
