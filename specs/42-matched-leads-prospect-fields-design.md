...
- **`frontend/src/features/signals/lib/signalBriefing.ts`** (`keyFindings` map) — each entry becomes a line built from non-empty parts, e.g. `"{name} — {title}, {seniority} ({company}) (Relevance: {High}): {why}"`. The identity prefix is added **only when present**, and the existing parenthesized `(Relevance: X)[: {why}]` wrapping is **preserved** so prospect-less leads render exactly as today. `signalBriefing.ts` has **two** builders rendering the same `SignalLeadMapLead[]` line — `buildSignalBriefingArtefact` (Spec 38) and `buildRecommendationPlaybookArtefact` (Spec 41) — so both route through one shared `formatLeadFinding` helper for consistency. The same `leads` array still feeds card + PDF, so `handleSaveAsArtefact` is structurally unchanged.
