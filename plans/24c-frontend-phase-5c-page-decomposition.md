...
- [ ] **Step 1: Plan the mount smoke test (written *after* extraction, run in Step 5) — not test-first.** Task 1 is a **verbatim move**, so there is no new behavior to drive via TDD: the moved code's characterization is the **existing page-level Vitest suite + `journeys/04`**, which already exercise it. A failing-test-first cycle here would force you to guess `IntelligenceTab`'s prop surface before the extraction determines it, and to stand up MSW handlers for all 9 moved fetch sites — ceremony with no design payoff. Instead, **after Step 3** add one minimal mount **smoke** test: render `<IntelligenceTab>` with the providers it ends up needing (at minimum a router context, since it renders nav-aware children) + MSW stubs for **only** the fetches it fires on mount, and assert it renders the intelligence surface (a known section heading or the "Load Data" CTA branch) without crashing. It runs as part of the Step 5 suite — its job is to catch a broken extraction, not to drive design. (Reserve real characterization TDD for Task 2's sanitizer, where it is concrete and valuable.)
...
- [ ] **Step 1a: Name the `scoutResearchContext` type.** The page declares it as an inline `useState<{…}>` literal (page L392) — there is no named type to import. Extract that exact object shape to a named type so both the page and `TrendsTab` reference one source: add `export interface ScoutResearchContext { /* the exact fields from the page L392 useState generic */ }` to `frontend/src/features/market-research/types.ts`, and change the page's `useState<…>` to `useState<ScoutResearchContext | null>`. (Copy the field list verbatim from the page's current generic — do not invent fields.)
  ...
  **An identical type already exists in legacy — do not import it.** `ChatWithScout.tsx` defines `ChatWithScoutResearchContext` with the same shape (`leads`, `opportunity?`, `icp?`, `reportTraits?`). Importing it would point the feature *at* a **leaving** legacy component — the wrong dependency direction (the same one-way feature→legacy boundary Task 3 enforces, in reverse). Instead create the **feature-local `ScoutResearchContext` and accept the duplication**: the feature owns its own types and `ChatWithScout` is slated to leave (Phase 7+). Structural typing means `TrendsTab` can still pass a `ScoutResearchContext` value into `ChatWithScout`'s `researchContext` prop **with no cast** (identical shapes are assignable). Add a one-line comment on the new interface noting it intentionally mirrors `ChatWithScoutResearchContext` until that component is retired.
...
- [ ] **Step 1c: Move `signalsChatContext` into `TrendsTab` (decided — not left in-flight).** The `signalsChatContext` `useState` (page L391) + its loader effect (page L414–430, which reads `sessionStorage.getItem("signalsChatContext")` *gated on* `activeTab === "trends"`) + its clear (`sessionStorage.removeItem` + `setSignalsChatContext(null)`) all move **into** `TrendsTab` and become its internal state — `signalsChatContext` has a single consumer (the trends chat), so it needs no hoist, just relocation. **Already resolved by the Task-0 inventory (no in-flight decision):** the out-of-band block renders **conditionally** — `{activeTab === "trends" ? (<trends chat>) : (<other-tabs ScrollArea>)}` (Key facts; page L6494) — i.e. the trends subtree **mounts/unmounts** on tab change, it is not CSS-hidden. (The empty `TabsContent value="trends"` placeholder at L6930 is the CSS-`hidden` one — but that is the dead placeholder being removed, not the real render.) So `TrendsTab` mounts only when trends is active, the `activeTab === "trends"` guard becomes **implicit**, and **no `isActive` prop is needed**. (Defensive fallback only: if a *future* change converts the branch to an always-mounted CSS-hide, re-add an `isActive` prop and keep the `activeTab` guard inside `TrendsTab`.)

- [ ] **Step 2: Route the page's `trends` branch to `<TrendsTab/>`.** Replace the out-of-band block (L6494–6511) with `<TrendsTab scoutResearchContext={scoutResearchContext} scoutMode={scoutMode} />`, and **remove the empty `TabsContent value="trends"` placeholder** (L6930–6933). The page still holds `scoutResearchContext`/`scoutMode` `useState` at this point (rehomed in Task 5). Remove the page's `signalsChatContext` `useState` + its loader effect (they moved into `TrendsTab` in Step 1c).

  **Verify the trends trigger still navigates once its panel is gone.** Removing `TabsContent value="trends"` leaves the Radix `Tabs` with no panel for that value. The controlled trigger (`value={activeTab}` / `onValueChange={handleTabChange}`) fires `onValueChange` regardless of a matching panel, so this *should* be a no-op — but the trends tab's only nav path is that trigger and a regression here is invisible to `tsc`/lint. Confirm in Step 3 that `journeys/04` actually **clicks** the trends (`chatwithscout`) `TabsTrigger` and lands on the scout-chat surface; if the journey only navigates by URL, add an explicit assertion (or a quick manual check) that clicking the trends `TabsTrigger` navigates and renders `TrendsTab`.
...
- [ ] **Step 2: Reduce the page to a thin shell.** `MarketResearchPage.tsx` now: read `activeTab` from the path → render `Layout` + the tab router that routes to `<IntelligenceTab/>` / `<LeadStreamTab .../>` (legacy, with the 3 shell-owned callbacks) / `<TrendsTab .../>` (with the shared pair as props); hold the `scoutResearchContext`/`scoutMode` `useState` + the three `handle*` handlers (shell-owned coordination). No fetch-result/server state in the shell's own `useState` (it all moved into `IntelligenceTab` in Task 1). Keep the existing `Tabs`/`TabsList`/`TabsTrigger` chrome that drives navigation.
  ...
  **Removal checklist (makes the "thin shell" claim diff-auditable):** after the prior tasks, explicitly delete anything left behind that only served the moved code — (a) imports of symbols that moved into `IntelligenceTab`/`LeadStreamTab`/`TrendsTab` (watch for destructured or re-exported forms a plain name-grep misses), (b) helper functions now unreferenced, (c) `useState`/`useEffect`/`useRef` whose only consumers left with the moved code. Then assert the shell's import block contains **only**: the router (`useNavigate`/`useLocation`), `Layout`, the three tab containers, the shared types (`ScoutResearchContext`), the `Tabs` chrome, and whatever the three shell-owned handlers depend on. `tsc`/eslint catch unused *imports*; this checklist targets the dead *helpers* and stale state they miss.
