...
- Whether `MarketResearchContext` exists at all, and its shape (Task 4 criteria).
- Search/filter state → URL vs local + history mode (Task 4 — but note the filters live on the *analysis* tab, which **leaves**; see Task 3).
...
test -f frontend/src/features/market-research/hooks/useMarketResearch.ts && echo "OK: 5b merged" || echo "STOP: 5b not merged"
...
- [ ] **Step 2: Create `IntelligenceTab.tsx`** — lift the `TabsContent value="intelligence"` subtree + the intelligence-only ephemeral `useState` (expand toggles, edit/dialog state) from the page into this container. It composes `MarketIntelligenceSections` (or its successor) and consumes 5b hooks for data. Keep markup identical (behavioral + visual parity via `journeys/04`).
...
```bash
cd /projects/Brewra/brewra-gtm-intelligence/frontend
npx eslint --fix src && npm run lint && npx tsc --noEmit -p tsconfig.app.json && npx vitest run src/features/market-research/components/intelligence
```
```bash
cd /projects/Brewra/brewra-gtm-intelligence
...
- [ ] **Step 1: Wrap the intelligence content in `<FeatureErrorBoundary>`** inside `IntelligenceTab` (or rely on the page-level boundary from 5a if a single boundary suffices — decide per the error-isolation granularity you want; default: a section-level boundary in `IntelligenceTab` so one section's crash doesn't blank the tab).

- [ ] **Step 2: Remove `SafeMarketIntelligenceTab`** and any now-orphaned thin wrapper (`MarketIntelligenceTab` if it only existed to feed Safe). Confirm no remaining importer:
```bash
cd /projects/Brewra/brewra-gtm-intelligence/frontend
...
## Task 4: Extract `TrendsTab` + rehome remaining state; decide on `MarketResearchContext`
...
> Spec 24 §2.1, §5. `trends` renders **Scout chat** (`ChatWithScout` / `ScoutChatWithHistory`) — both legacy/leaving (scout / signals). `TrendsTab` is therefore a **thin router** over those legacy components (transitional import), not a genuine MR view. Then apply the state-rehoming criteria to whatever `useState` remains in the page.
...
- [ ] **Step 2: Apply the state-rehoming criteria to the page's residual `useState`** (Spec 24 §5 — precedent for Phases 6–12):
...
- [ ] **Step 3: Reduce the page to a thin shell** — `MarketResearchPage.tsx` now: read `:tab` → route to `<IntelligenceTab/>` / `<LeadStreamTab/>` (legacy) / `<TrendsTab/>`; render `Layout`; the 5a `<FeatureErrorBoundary>` wrap stays at the route. No fetch-result/server state in page `useState`.
...
cd /projects/Brewra/brewra-gtm-intelligence
git add -A frontend/src
git commit -m "refactor(fe): thin MarketResearchPage shell + TrendsTab router; rehome residual state (context decision recorded)"
```
...
- **Spec coverage:** §5 thin shell + tab containers (Tasks 1, 4); state rehoming criteria + context decision (Task 4); URL-vs-local filter constraint (Task 4 — noting filters leave with analysis); Safe→FeatureErrorBoundary (Task 2); inline analysis extraction to legacy + one-way boundary + shared-GET default (Task 3); §5 "Done when" (Task 5).
