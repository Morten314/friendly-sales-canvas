...
const MiniContextualReport = () => {
  ...
  const LinkedInIcon = ({
    ...
    onClick: () => void;
    className?: string;
  }) => <button onClick={onClick} className="inline-flex items-center justify-center hover:opacity-80 transition-opacity" title="View LinkedIn Profile">
      <svg className={className} fill="#0A66C2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  return <div className="w-full space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {reportCards.map(card => <Card key={card.id} className="hover:shadow-md transition-shadow">
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {card.keyTraits.map((trait, index) => <Badge key={index} variant="secondary" className="text-xs">
                      {trait}
                    </Badge>)}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {card.painPoints.map((point, index) => <Badge key={index} variant="outline" className="text-xs bg-red-50 text-red-700 border-red-200">
                      {point}
                    </Badge>)}
                </div>
              </div>
      ...
      <div className="flex justify-end">
        <Button variant="outline" onClick={() => setShowAll(!showAll)} className="text-blue-600 border-blue-200 hover:bg-blue-50">
      ...
      {showAll && <Card className="mt-4 bg-gray-50/50">
          <CardHeader className="pb-3">
          ...
          <CardContent>
            <div className="bg-white rounded-md border">
              <Table>
                ...
                <TableBody>
                  {prospectsList.map(prospect => <TableRow key={prospect.id} className="hover:bg-gray-50">
                      <TableCell className="font-medium">
                        <div className="flex items-center gap-2">
                          {prospect.fullName}
                          <LinkedInIcon onClick={() => handleLinkedInClick(prospect.linkedinProfile)} className="h-3 w-3" />
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <LinkedInIcon onClick={() => handleLinkedInClick(prospect.linkedinCompany)} className="h-3 w-3" />
                        </div>
                      </TableCell>
                      <TableCell className="text-gray-600">{prospect.designation}</TableCell>
                      <TableCell className="text-center">
                        <Button variant="ghost" size="sm" onClick={() => handleLinkedInClick(prospect.linkedinProfile)} className="text-blue-600 hover:text-blue-800">
