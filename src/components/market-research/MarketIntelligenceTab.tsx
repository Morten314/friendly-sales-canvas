...
const MarketIntelligenceTab: React.FC<MarketIntelligenceTabProps> = ({
  ...
  const handleCompetitorSaveChanges = () => {
    // Save the changes first
    onCompetitorSaveChanges();
    // Then trigger the Scout chat panel with competitor context
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 relative">
        ...
        {isCompetitorEditing ? (
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    disabled={competitorEditHistory.length === 0}
                  >
                    <Clock className="h-4 w-4" />
                    Edit History
                  </Button>
          ...
          <div className="space-y-6">
            ...
            {/* Read More Button */}
            {!competitorExpanded && (
              <Button variant="secondary" onClick={() => setCompetitorExpanded(true)} className="w-full">
                Read More
              </Button>
            )}
