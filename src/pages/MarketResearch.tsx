...
const MarketResearch = React.memo(() => {
  ...
  const handleChatWithScout = (leads: any[], reportFilter?: string) => {
    ...
    if (reportFilter) {
      ...
      if (reportFilter === 'market-size') addMarketSizeTraits();
      else if (reportFilter === 'industry-trends') addIndustryTraits();
      else if (reportFilter === 'competitor-landscape') addCompetitorTraits();
      else if (reportFilter === 'regulatory-compliance') addRegulatoryTraits();
      else if (reportFilter === 'market-entry') addMarketEntryTraits();
    } else {
      // No filter — aggregate traits from all available reports
      addMarketSizeTraits();
      addIndustryTraits();
      addCompetitorTraits();
      addRegulatoryTraits();
      addMarketEntryTraits();
    }
            ...
            <Tabs value={activeTab} onValueChange={handleTabChange} className="mt-0 h-full min-h-0 flex flex-col">
              ...
              <TabsContent value="analysis" className="mt-0">
                ...
                <ScoutLeadStream 
                  ...
                  onQualifyProfile={(leads) => {
                    const leadArray = Array.isArray(leads) ? leads : [leads];
                    console.log('Qualify Profile:', leadArray.length, 'leads');
