...
export const buildArtefactPdfDoc = (artefact: ArtefactItem): jsPDF => {
  ...
  if (answers.length > 0) {
    writeBlock("RECOMMENDATION DEEP DIVES", 13, true);
    answers.forEach((qa, i) => {
      writeBlock(`${i + 1}. ${qa.question}`, 11, true);
      writeBlock(sanitizeAnswerText(qa.answer), 10, false);
      gap(8);
    });
    gap(8);
  }
  ...
  if (sequence.length > 0) {
    writeBlock("DETAILED OUTREACH PLAN", 13, true);
    sequence.forEach((touch) => {
      writeBlock(`Day ${touch.day} - ${touch.channel} - ${touch.action}`, 11, true);
      if (touch.subject) writeBlock(`Subject: ${touch.subject}`, 10, false);
      writeBlock(touch.body, 10, false);
      gap(8);
    });
    gap(8);
  }
