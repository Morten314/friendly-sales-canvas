...
## Domains
`icp`, `signals`, `leads`, `market_research`, `market_scoring`, `customer_profile`, `data_sources`, `org_auth`, `graph_chat`, `pipeline`, `profiles`, `connectors` (twelve domain service packages under `app/services/`; the first eleven each expose a v1 router, and `connectors` is the Apollo lead-discovery integration — `app/services/connectors/` with router `app/routers/connectors.py` mounted at `/connectors`, added in Spec 35). Plus `health` — a service module (`app/services/health.py`) with no dedicated router; see §Health below.
...
- **LLMs** (`app/core/llm_config`): Together.ai `Qwen/Qwen3-235B-A22B-Instruct-2507-tput` is the single non-Claude chat model (also drives the `LLMGraphTransformer`, the Apollo discovery re-rank, and prospect scoring), exposed as a `ChatOpenAI` pointed at `https://api.together.xyz/v1`, used in a LangChain `ZERO_SHOT_REACT_DESCRIPTION` `agent_chain` (`max_iterations=20`, `max_execution_time=120`) with a Tavily `WebSearch` tool (`k=10`). Anthropic Claude (`claude-sonnet-4-6`, via `config.claude_sonnet_model`) backs the `_claude` signal/ask endpoints the frontend calls for Scout/Profiler. **Groq `llama-3.3-70b-versatile` was retired 2026-06-14** — every path it drove moved to Qwen (validated live before the swap). Embeddings (`app/services/_retrieval.py`) use `intfloat/multilingual-e5-large-instruct` (1024-dim) served by Together.ai via `langchain_openai.OpenAIEmbeddings` (also pointed at `https://api.together.xyz/v1`) — **not** OpenAI despite the class name.
