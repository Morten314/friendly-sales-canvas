...
- **Prompts live in `backend/prompts/<svc>/`** (Jinja2 bodies) served by `app/core/prompts.py`; regional-bias examples (APAC, NA/DACH, healthcare) are in the prompt bodies themselves. When research output looks biased, the prompt is the cause — see `docs/PROMPTS.md` and `backend/ANALYSIS_MARKET_ICP_RESEARCH_ISSUES.md`.
